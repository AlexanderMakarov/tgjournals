...
/**
 * Service for validating Telegram webhook requests using secret token
 * authentication. This ensures that only Telegram can send requests to our
 * webhook endpoint.
 */
@Service
...
public class WebhookSecurityService {
	...
	public ValidationResult validateWebhookRequest(HttpServletRequest request) {
		...
		if (!StringUtils.hasText(webhookSecret)) {
			return ValidationResult.success("No webhook secret configured - allowing all requests (development mode)");
		}
